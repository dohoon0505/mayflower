...
│  │  (사무직 전용)   │  │  (기사 전용)      │  │  (기사 전용)        │  │
│  │                 │  │                 │  │                     │  │
│  │  - 2층 수주      │  │  ✅ 내 배송 목록  │  │  ✅ 내 배송 목록    │  │
│  │  - 1층 제작      │  │  ✅ GPS 위치추적  │  │  ✅ GPS 위치추적    │  │
│  │  - 관리자 대시보드 │  │  ✅ 카메라 촬영   │  │  ✅ 카메라 촬영     │  │
│  │  - (기사 Web 접속│  │  ✅ 푸시 알림     │  │  ✅ 푸시 알림       │  │
│  │    은 차단됨)    │  │  ❌ 관리/통계 없음│  │  ❌ 관리/통계 없음  │  │
│  └────────┬────────┘  └────────┬────────┘  └──────────┬──────────┘  │
│           │                    │                      │             │
└───────────┼────────────────────┼──────────────────────┼─────────────┘
            │                    │                      │
...
> ⚠️ **핵심 원칙:** Android / iOS 앱은 **배송 기사 전용**입니다.  
> 사무직(floor2, floor1, admin) 역할은 반드시 **Flutter Web**으로만 접속합니다.  
> 모바일 앱에서 기사가 아닌 계정으로 로그인하면 **즉시 차단**됩니다.

| 사용자 | 플랫폼 | 접근 가능 화면 |
|--------|--------|----------------|
| 2층 수주 담당 (floor2) | **Flutter Web 전용** | 신규 주문 접수, 내 주문 조회, 채팅 |
| 1층 제작 담당 (floor1) | **Flutter Web 전용** | 전체 주문 조회, 기사 배정, 채팅 |
| 관리자 (admin) | **Flutter Web 전용** | 모든 화면 + 배송패널 + 통계/관리 |
| 배송 기사 (driver) | **Flutter Mobile 전용** (Android/iOS) | 내 배달 목록, 주문 상세, 카메라, GPS |

### 플랫폼 분리 이유

| 이유 | 설명 |
|------|------|
| **UX 최적화** | 사무직은 키보드·큰 화면 필요, 기사는 한 손 조작·심플 UI 필요 |
| **네이티브 기능** | 카메라·GPS·백그라운드 위치·푸시 알림은 Web에서 제한적 |
| **보안** | 기사가 관리/통계 화면에 접근하는 것 자체를 앱 레벨에서 차단 |
| **번들 크기** | 기사 앱에 불필요한 관리자 화면 코드가 포함되지 않음 |
...
| 기능 | Web (사무직 전용) | Mobile Android/iOS (기사 전용) |
|------|-----------------|-------------------------------|
| **접근 역할** | floor2, floor1, admin (driver 차단) | driver 전용 (다른 역할 차단) |
| **레이아웃** | 4열 그리드 대시보드 | 단일 열 + 하단 탭 네비게이션 |
| **사이드바** | 항상 표시 | 없음 (BottomNavigationBar 사용) |
| **배송 패널** | 관리자용 우측 고정 패널 | 없음 (기사 화면에 불필요) |
| **채팅** | 메인 화면 우측 패널 | 별도 탭 화면 |
| **카메라** | 웹 카메라 (image_picker_for_web) | 네이티브 카메라 (image_picker) |
| **GPS** | 브라우저 Geolocation API (제한) | 네이티브 GPS (geolocator) |
| **푸시 알림** | 브라우저 Notification (Service Worker) | FCM 네이티브 (flutter_local_notifications) |
